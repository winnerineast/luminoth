...
@click.command()
@click.option('--data-dir', default='datasets/coco')
@click.option('--output-dir', default='datasets/coco/tf')
...
def coco(data_dir, output_dir, splits, ignore_splits, only_filename,
         limit_examples, limit_classes, seed, debug):
    """
    Prepare COCO dataset for ingestion.

    Converts the COCO dataset into three (one per split) TFRecords files.
    """
    if debug:
